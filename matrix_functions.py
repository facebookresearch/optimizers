    ...
    match eigendecomposition_config:
        case EighEigendecompositionConfig():
            if eigendecomposition_config.tolerance != 0.0:
                ...
                if not isinstance(eigenvectors_estimate, torch.Tensor):
                    raise ValueError(
                        f"{type(eigenvectors_estimate)=} should be Tensor when using tolerance != 0.0."
                    )
                eigenvalues_estimate = (
                    eigenvectors_estimate.T @ A @ eigenvectors_estimate
                )
        ...
        case _:
            raise NotImplementedError(
                f"Eigendecomposition config is not implemented! Specified eigendecomposition config is {eigendecomposition_config.__class__.__name__}."
            )
