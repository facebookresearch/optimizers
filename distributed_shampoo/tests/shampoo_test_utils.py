...
def train_model(
    ...
    # Move data onto the model's device. Pre-generated global data (e.g. the CPU-vs-device
    # comparisons) is created once on a single device for value-determinism across devices,
    # so the control model on CPU must relocate its data here to avoid a device mismatch.
