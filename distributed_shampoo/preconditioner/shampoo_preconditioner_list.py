...
class BaseShampooPreconditionerList(
    ...
    def _precondition_grad(
        ...
        # Create a separate iterator just for peeking at dtypes
        dtype_iter = iter(preconditioner_list)

        return reduce(
            lambda grad, should_precondition: torch.tensordot(
                # Peek at the next preconditioner's dtype without consuming the main iterator
                grad.to(
                    dtype=next(dtype_iter).dtype if should_precondition else grad.dtype
                ),
