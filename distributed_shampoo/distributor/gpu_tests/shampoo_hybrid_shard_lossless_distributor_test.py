...
@instantiate_parametrized_tests
class ShampooHybridShardLosslessDistributorTest(DTensorTestBase):
    @property
...
class HybridShardLosslessDistributorOnEmptyParamTest(
    ...
    def _construct_model_and_distributor(
        self,
    ) -> tuple[nn.Module, HybridShardLosslessDistributor]:
        device_mesh = init_device_mesh(
            "cuda", (2, 2), mesh_dim_names=("replicate", "shard")
        )
        ...
        # With param assignment strategy REPLICATE, we will have all parameters replicated in the two replicate groups.
        # Within each replicate group, there will be three blocks split into two ranks in that replica group:
        # one having two blocks, and another having one block, all blocks having the same shape torch.Size((PRECONDITIONER_DIM, PRECONDITIONER_DIM))
        ...
        distributor = HybridShardLosslessDistributor(
